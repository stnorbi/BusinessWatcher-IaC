...
# terragrunt output -raw temp_admin_access_key > ./temp_admin_AK.txt
# terraform output -json temp_admin_secret_key | jq -r . | base64 --decode --ignore-garbage | gpg --enarmor > ./temp_admin_SK.txt

ADMIN_ACCESS_KEY=$(terraform output -raw temp_admin_access_key)
ADMIN_SECRET_KEY=$(terraform output -raw temp_admin_secret_key | base64 --decode --ignore-garbage | keybase --pinentry=none pgp decrypt)
popd
...
pushd ./eu-central-1/deployment/codecommit
